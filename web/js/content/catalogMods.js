var catalogMods = {}; // Update Solutioin Spring 2022
...
(function () {  // This is an IIFE, an immediately executing function.
    ...
    catalogMods.insert = function () {

        // Create UI for insert (using the shared DOM elements just above, in the IIFE).
        userEditArea.areaTitle.innerHTML = "New Catalog Entry";
...
//            userInputObj.userRoleId = roleSelect.options[roleSelect.selectedIndex].value;
            ...
            ajax("webAPIs/insertCatalogAPI.jsp?jsonData=" + urlParams, reportInsert, userEditArea.formMsg);
    ...
    catalogMods.update = function (bookId) {

        userEditArea.areaTitle.innerHTML = "Update Catalog";
        userEditArea.blankInputs();
        userEditArea.button.innerHTML = "Update Save";
        userEditArea.formMsg.innerHTML = ""; // wipe out any old message

        console.log("catalogMods.update called with bookIs " + bookId);

        // get the web user record with the given webUserId
        ajax("webAPIs/getCatalogByIdAPI.jsp?bookId=" + bookId, gotRecordById, userEditArea.formMsg);

        // webUserObj is the output of getUserByIdAPI.jsp
        function gotRecordById(catalogObj) {

            userEditArea.writeDbValuesToUI(catalogObj);
...
//                } else {
//                    console.log("userRoleId is " + catalogObj.userRoleId);
//                    var selectTag = Utils.makePickList({
//                        list: obj.roleList,
//                        idProp: "userRoleId",
//                        displayProp: "userRoleType",
//                        selectedKey: catalogObj.userRoleId  // key that is to be pre-selected (optional)
//                    });
//
...
//            userInputObj.userRoleId = roleSelect.options[roleSelect.selectedIndex].value;
            ...
            ajax("webAPIs/updateCatalogAPI.jsp?jsonData=" + urlParams, reportUpdate, userEditArea.formMsg);
